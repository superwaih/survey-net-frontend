...
const ChooseUs = () => {
  return (
    <div className="flex flex-col  text-[#666] py-12 w-full">
      ...
      <div className='py-12 px-8 md:px-12  grid grid-cols-1 md:grid-cols-2 gap-5 '>
            {choose.map((place) => {
                const {id, title, content, Icon} = place
                return(
                    <div key={id} className='flex w-fit  flex-col py-3 px-3 border-[#ebebeb]  text-[#666] hover:scale-105 gap-4 cursor-pointer transition-all duration-300 bg-white rounded-lg shadow-md border'>
